...
namespace numth {
    ...
    inline ll inv(ll a, ll mod_prime) { 
        return pow(a, mod_prime - 2, mod_prime);
    }
}
```
在**模数为质数**的情况下, 模运算构成一个环.
...
## 埃氏筛
在不需要处理积性函数的时候, 埃氏筛远比欧拉筛好用. 虽然欧拉筛渐进意义上更优, 但埃氏筛由于极低的常数, 缓存友好, 连续读写, 使用bitset后在算法竞赛中实际上是比欧拉筛更快的.
```cpp
...
### 分段埃氏筛
```cpp
bitset<SQN + 5> not_prime;
void sieve(int n = SQN) {
    // 先筛出到sqrt(n)为止的质数
    for (int i = 2; i <= SQN; i++) {
        if (not_prime[i]) {
            continue;
        }
        for (int j = 2; i * j <= SQN; j++) {
            not_prime[i * j] = 1;
        }
    }
}

vector<bool> era(int l, int r) {
    // [l, r] 上的所有质数
    for (int i = 2; i * i < r; i++) {
        if (not_prime[i]) {
            continue;
        }
        for (int j = (l + i - 1) / i; 1LL * i * j < r; j++) {
...
int d[N + 5], times[N + 5];     // 求d用, 表示最小质因子的次数.

ll sigma[N + 5];
int g[N + 5];            // 求sigma用, 表示最小质因子的幂次贡献的约数和.
*/
void sieve(int n = N) {
    /*
    phi[1] = 1;
...
void sieve(int n = N) {
    ...
    for (int i = 2; i <= n; i++) {
        ...
        for (int j = 0; j < primes.size() && i * primes[j] <= n; j++) {
            ...
            int tgt = primes[j] * i;
            if (i % primes[j] == 0) {
                low[tgt] = primes[j] * low[i];
                if (i == low[i]) {
                    f[tgt] = getf(primes[j], low[tgt]);
                }
                else {
