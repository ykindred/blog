...
ll MST_Kruskal(int cntv, vector<edge> e) {
    if (cntv == 1) {
        return 0;
    }
    int n = cntv;
    ...
    for (const auto& [u, v, w] : e) {
        if (ufs.merge(u, v)) {
            ...
            // adj[u].emplace(v, w), adj[v].emplace(u, w)建图
        }
    }
    return -1; // 无最小生成树
}
...
bset MST_Kruskal(int cntv, vector<edge> e) {
    sort(e.begin(), e.end());
    int n = cntv;
    int m = e.size();
    int cnt = 0;
    UfSet ufs(n);
    bset bs;
    for (int i = 0, r = 1; i < m; i = r, r = i + 1) {
        vector<edge> tmp{ e[i] };
        while (r < m && e[r].w == e[i].w) {
            tmp.emplace_back(e[r]);
            r++;
        }
        for (const auto& [u, v, w, id] : tmp) {
            if (!ufs.same(u, v)) {
                bs[id] = 1;
            }
        }
